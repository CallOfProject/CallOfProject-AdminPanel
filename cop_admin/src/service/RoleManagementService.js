...
export const giveAdminRole = async (username) => {
    try {
        const GIVE_ROLE_URL = `${URL_PREFIX}/api/root/give/role/admin?username=${username}`
...
export const removeAdminRole = async (username) => {
    try {
        const REMOVE_ROLE_URL = `${URL_PREFIX}/api/root/remove/role/admin?username=${username}`
        const token = getUserInformationFromLocalStorage().accessToken;
